  ...
  try {
    ...
    const result = await pool.query(
      `SELECT ci.id, ci.variant_id, ci.quantity, v.sku, v.color_name, p.name, p.base_price, p.discount_percent, v.stock_quantity,
       ...
       ((p.base_price + COALESCE(v.price_modifier, 0)) * (1 - p.discount_percent / 100)) AS final_unit_price,
       img.image_url
       FROM cart_items ci
...
export const addToCart = async (req, res, next) => {
  try {
    ...
    const stockQuantity = variantCheck.rows[0].stock_quantity;
...
export const updateCartItem = async (req, res, next) => {
  try {
    ...
    if (quantity > itemCheck.rows[0].stock_quantity) {
      return res.status(400).json({ message: "Requested quantity exceeds available stock" });
    }
